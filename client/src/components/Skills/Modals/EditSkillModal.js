...
import { createSkillArgs } from '../../../graphQLOps/mutation/createSkill';
import { updateSkillArgs } from '../../../graphQLOps/mutation/updateSkill';

export const EditSkillModal = ({setIsModalVisible, isModalVisible, skillToEdit, updateSkill  }) => {

    const [oldSkillName, setOldSkillName] = useState(skillToEdit.name);
    const [skillName, setSkillName] = useState(skillToEdit.name);
    const [description, setDescription] = useState(skillToEdit.description);
    const [imageLink, setImageLink] = useState(skillToEdit.imageURL);

    useEffect(() => {
        setOldSkillName(skillToEdit.name)
        setSkillName(skillToEdit.name)
        setDescription(skillToEdit.description)
        setImageLink(skillToEdit.imageURL)
    }, [skillToEdit])
